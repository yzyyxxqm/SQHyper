...
from layers.Pathformer.layers.Layer import Transformer_Layer
from layers.Pathformer.utils.Other import (
    MLP,
    FourierLayer,
    SparseDispatcher,
    series_decomp_multi,
...
class AMS(nn.Module):
    def __init__(self, input_size, output_size, num_experts, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        # min_length = min(output.shape[1], x.shape[1])
        # if self.residual_connection:
        #     output = output[:, :min_length] + x[:, :min_length]
        if self.residual_connection:
            output = output + x
