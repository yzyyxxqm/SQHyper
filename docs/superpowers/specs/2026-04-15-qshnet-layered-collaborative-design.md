...
## 8. M1 实施后的结果回看（2026-04-15）

### 8.1 已完成的 M1 改动

设计中的 M1 已按最小版本落地，实际实现包括：

- **SpikeRouter：** 输出 `obs_base`、`obs_event` 和 `route_state`。
- **双通道 `n2h`：** temporal / variable hyperedge 接收零初始化的事件增强残差流。
- **条件化 Quaternion gate：** 用节点级 gate 替换全局 `alpha`。

### 8.2 实验结果摘要

服务器实验结果表明：

- **USHCN（10 轮）：** `0.1846 ± 0.0323`，中位数 `0.1689`
- **HumanActivity（5 轮）：** `0.0416 ± 0.0002`
- **P12（5 轮）：** `0.3012 ± 0.0012`
- **MIMIC_III（5 轮）：** `0.4047 ± 0.0300`，中位数 `0.3929`
- **MIMIC_IV：** 暂未运行

与设计前的预期相比，当前结果呈现出一个明确特征：

> **M1 已证明“结构可安全接入”，但尚未证明“机制能稳定提升”。**

### 8.3 哪些预期被验证了

- **安全退化基本成立。**
  - HumanActivity 基本持平，P12 仅小幅波动，说明简单数据集没有被新结构明显破坏。

- **新结构不是方向性错误。**
  - USHCN 和 MIMIC_III 的好轮次仍然存在，说明主干 + 路由 + 条件 gate 的组合不是无效设计。

- **“先保住主干，再让增强渐进参与”的原则是正确的。**
  - 至少从跨数据集退化安全性看，M1 优于此前那些直接重写主干机制的激进尝试。

### 8.4 哪些问题暴露出来了

- **USHCN 的双峰问题没有被解决。**
  - 多数轮次已处在较好区间，但仍存在明显异常轮，说明新分支没有根治高方差问题。

- **MIMIC_III 出现偶发坏轮。**
  - 好轮次接近旧版强区间，但均值和 std 明显变差，说明新机制在部分 seed 上会把训练带偏。

- **当前主要矛盾是稳定性，而不是表达力。**
  - 这批结果不支持立刻继续扩展 M2，因为问题不在“能力不够”，而在“偶发失稳”。

### 8.5 对原设计的修正解释

因此，这份设计文档在实施后应补充一个现实修正：

- **M1 的角色应从“最小可行升级版”调整为“安全重构基线”。**
- **M2 暂不推进。** 在 M1 尚未稳定前，不宜继续增加事件统计注入、细粒度 gate 或额外正则。
- **下一步应优先做保守稳定化。** 核心对象不是继续扩结构，而是诊断并限制 `event_gate`、`event_residual_scale`、`quat_gate` 的异常放大。

### 8.6 当前结论

到 2026-04-15 晚间为止，可以对方案 B 给出如下结论：

> **方案 B 的方向是成立的，但 M1 当前仍停留在“正确方向上的第一版原型”，而不是最终可定版结果。**

这意味着：

- 这条路线值得保留；
- 当前版本不应回退；
- 但下一阶段目标应从“继续加功能”切换为“先稳住已有结构”。
