...
## 分层协同型 M1 实施结果（2026-04-15，当前保留）

### 实现摘要

基于方案 B 的最小落地版本 M1 已完成实现并提交（commit: `6390ec5`），包含以下 3 个改动：

- **SpikeRouter：** 将原始 `SpikeSelection` 升级为连续路由器，输出 `obs_base`、`obs_event` 和 `route_state`。
- **双通道 `n2h`：** 在 temporal / variable hyperedge 更新中加入零初始化的事件增强残差流。
- **条件化 Quaternion gate：** 将全局标量 `alpha` 改为节点级条件化 gate。

本地单元测试已通过，训练实验由服务器执行。

### 多数据集实验结果

| 数据集 | 轮数 | MSE 均值 ± std | 中位数 | 最优 | 最差 | 对旧版 QSH-Net 的判断 |
|--------|------|----------------|--------|------|------|----------------------|
| USHCN | 10 | **0.1846 ± 0.0323** | 0.1689 | 0.1622 | 0.2626 | 均值略优于旧版 0.1862，但双峰和异常轮仍在 |
| HumanActivity | 5 | **0.0416 ± 0.0002** | 0.0417 | 0.0414 | 0.0418 | 基本持平，说明简单数据上安全退化成立 |
| P12 | 5 | 0.3012 ± 0.0012 | 0.3008 | 0.2999 | 0.3027 | 略差于旧版 0.3006，但幅度很小 |
| MIMIC_III | 5 | 0.4047 ± 0.0300 | 0.3929 | 0.3865 | 0.4577 | 好轮次接近旧版，但均值与稳定性退步 |
| MIMIC_IV | - | 未运行 | - | - | - | 由于训练成本较高，暂未评估 |

### 核心判断

1. **结构方向正确，但收益尚未稳定。**
   - HumanActivity 与 P12 没有被明显打坏，说明 M1 的「安全退化」目标基本成立。
   - USHCN 与 MIMIC_III 的好轮次都保留了下来，说明新结构不是方向性错误。

2. **当前主要问题是偶发失稳，而不是表达力不足。**
   - USHCN 的多数轮次已落在 0.162~0.181 区间，但仍存在 0.2208 / 0.2626 的异常轮。
   - MIMIC_III 的 4/5 轮仍在旧版强区间附近，但有 1 轮明显崩坏到 0.4577。

3. **M1 更适合作为“安全重构基线”，而不是最终定版。**
   - 这版已经证明分层协同型设计能够安全接入 HyperIMTS 主干。
   - 但它还没有证明新机制可以稳定地产生跨数据集收益。

### 当前决策

- **保留 M1 版本，不回退。**
- **暂不进入 M2。** 当前不应继续扩充事件统计或更复杂 gate，而应优先稳定 M1。
- **下一步重点：** 围绕 `event_gate`、`event_residual_scale`、`quat_gate` 做诊断与保守稳定化，优先解决 USHCN 的双峰和 MIMIC_III 的异常坏轮。
