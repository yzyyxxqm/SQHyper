...
def get_configs(args=None) -> ExpConfigs:
    ...
    parser.add_argument('--use_compile', type=int, default=0, help='Whether to wrap the model with torch.compile (PyTorch 2.x graph compiler). Adds ~30s first-batch overhead but typically gives 20-40%% speedup on subsequent batches. Set to 1 to enable, 0 to disable. Uses dynamic=True and mode="default" for IMTS variable-length compatibility.')
    parser.add_argument('--use_amp', type=int, default=0, help='Whether to enable automatic mixed precision training. 0 = off (fp32). 1 = bfloat16 autocast (no GradScaler needed, safer for attention). 2 = float16 autocast + GradScaler (faster but tighter dynamic range). Recommended value on Ampere+ GPUs: 1.')
    ...
    parser.add_argument('--sthq_spike_floor', type=float, default=0.0, help='STHQ: minimum K/V gating weight on observed cells; prevents spike starvation on sparse medical data (0=disabled, recommended 0.1-0.2 for P12/MIMIC).')
    parser.add_argument('--sthq_event_layer_warmup', type=int, default=0, help='STHQ: zero out event features (q_K) for the first N layers, letting QMF first learn temporal/variable fusion before integrating event signal.')
    parser.add_argument('--sthq_no_spike', type=int, default=0, help='STHQ ablation: disable SpikeEncoder (mask-only K/V gating, zero event features).')
    parser.add_argument('--sthq_no_quaternion', type=int, default=0, help='STHQ ablation: replace QMF with flat Linear(D, D) for hyperedge -> node fusion.')
    parser.add_argument('--sthq_diag_interval', type=int, default=0, help='STHQ: log spike gate / event norm / gate_scale every N forward passes during training (0=disabled).')
    # SQHH: Spike-Quaternion Heterogeneous Hypergraph
    parser.add_argument('--sqhh_k_a', type=int, default=16, help='SQHH: number of Layer-2 quaternion anchors (global pattern hyperedges).')
    parser.add_argument('--sqhh_k_e', type=int, default=32, help='SQHH: number of Layer-1 spike-triggered event hyperedges per batch.')
    parser.add_argument('--sqhh_spike_floor', type=float, default=0.0, help='SQHH: minimum effective spike intensity (after refractory inhibition).')
    ...
    parser.add_argument('--eaqhg_k_e', type=int, default=32, help='EAQ-HG: number of spike-triggered event hyperedges per batch.')
    parser.add_argument('--eaqhg_k_a', type=int, default=16, help='EAQ-HG: number of learned quaternion anchor hyperedges (global pool).')
    parser.add_argument('--eaqhg_spike_floor', type=float, default=0.1, help='EAQ-HG: minimum effective spike intensity (after refractory).')
    parser.add_argument('--eaqhg_dt_aggr', type=float, default=0.05, help='EAQ-HG: initial half-width of event aggregation window (normalized time).')
    parser.add_argument('--eaqhg_dt_dist', type=float, default=0.1, help='EAQ-HG: initial half-width of event distribution window for h2n.')
    parser.add_argument('--eaqhg_no_events', type=int, default=0, help='EAQ-HG ablation: drop K_e spike-event hyperedges.')
    parser.add_argument('--eaqhg_no_anchors', type=int, default=0, help='EAQ-HG ablation: drop K_a learned anchor hyperedges.')
    parser.add_argument('--eaqhg_no_sqc', type=int, default=0, help='EAQ-HG ablation: skip Spike-Quaternion rotation (SQC).')
    parser.add_argument('--eaqhg_no_quat_fusion', type=int, default=0, help='EAQ-HG ablation: replace Hamilton fusion of (q_self, msg_e, msg_a) with flat Linear.')
    parser.add_argument('--eaqhg_no_quat_decode', type=int, default=0, help='EAQ-HG ablation: replace cross-attention decoder with mean-pool over h_e/h_a.')
    parser.add_argument('--eaqhg_no_sra', type=int, default=0, help='EAQ-HG ablation: disable refractory inhibition inside SpikeRefractoryEncoder.')
    parser.add_argument('--eaqhg_diag_interval', type=int, default=0, help='EAQ-HG: log spike + event/anchor stats every N forward passes (0=disabled).')
    # RFH: Receptive-Field Hypergraph (truly orthogonal to HyperIMTS; no quaternion, no spike top-K)
    parser.add_argument('--rfh_k_f', type=int, default=64, help='RFH: number of learned receptive-field hyperedges (continuous Gaussian regions in (t, v, value) space).')
    parser.add_argument('--rfh_k_e', type=int, default=32, help='RFH: number of salience-triggered event hyperedges.')
    parser.add_argument('--rfh_k_a', type=int, default=16, help='RFH: number of learned global anchor hyperedges.')
    parser.add_argument('--rfh_bw_t_init', type=float, default=0.1, help='RFH: initial time-bandwidth (normalized [0,1] units) of receptive fields.')
    parser.add_argument('--rfh_bw_x_init', type=float, default=0.5, help='RFH: initial value-bandwidth of receptive fields (assumes normalized values).')
    parser.add_argument('--rfh_dt_aggr', type=float, default=0.05, help='RFH: initial event aggregation window half-width.')
    parser.add_argument('--rfh_dt_dist', type=float, default=0.1, help='RFH: initial event distribution window half-width for h2n.')
    parser.add_argument('--rfh_no_field', type=int, default=0, help='RFH ablation: disable K_f receptive-field hyperedges.')
    parser.add_argument('--rfh_no_events', type=int, default=0, help='RFH ablation: disable K_e salience-event hyperedges.')
    parser.add_argument('--rfh_no_anchors', type=int, default=0, help='RFH ablation: disable K_a global anchor hyperedges.')
    parser.add_argument('--rfh_no_value_kernel', type=int, default=0, help='RFH ablation: remove value-axis Gaussian from receptive-field incidence (use only (t, var) Gaussian).')
    parser.add_argument('--rfh_no_salience', type=int, default=0, help='RFH ablation: salience identically 1 for all observed cells (events become spike-floor=1 picks).')
    parser.add_argument('--rfh_diag_interval', type=int, default=0, help='RFH: log field/event/anchor norms + salience stats every N steps (0=disabled).')
    # PSQH: Prototype Spiking Quaternion Hypergraph
    parser.add_argument('--psqh_n_prototypes', type=int, default=32, help='PSQH: number of learnable quaternion prototype hyperedges K.')
    parser.add_argument('--psqh_tau_spike_init', type=float, default=1.0, help='PSQH: initial temperature for spike sigmoid gate.')
    parser.add_argument('--psqh_tau_comp_init', type=float, default=0.5, help='PSQH: initial temperature for per-node competitive softmax.')
    parser.add_argument('--psqh_bw_t_init', type=float, default=0.2, help='PSQH: initial temporal bandwidth for prototype Gaussian windows (in [0,1] time).')
    parser.add_argument('--psqh_var_pref', type=int, default=1, help='PSQH: 1 = prototypes have per-variable soft preferences (joint time×var localisation). 0 = no var preference.')
    parser.add_argument('--psqh_no_node_attn', type=int, default=0, help='PSQH: 1 = disable per-block node self-attention (may help sparse datasets).')
    parser.add_argument('--psqh_no_spike', type=int, default=0, help='PSQH ablation: 1 = uniform 1/K incidence (no spike selection).')
    parser.add_argument('--psqh_no_quat', type=int, default=0, help='PSQH ablation: 1 = replace Hamilton products with standard Linear.')
    parser.add_argument('--psqh_lambda_div', type=float, default=0.1, help='PSQH: diversity loss weight for prototype anti-collapse.')
    parser.add_argument('--psqh_diag_interval', type=int, default=0, help='PSQH: log spike stats every N steps (0=disabled).')
    # SQEH: Spiking Quaternion Event Hypergraph
    parser.add_argument('--sqeh_n_events', type=int, default=64, help='SQEH: number of latent event hyperedges.')
    parser.add_argument('--sqeh_spike_slope', type=float, default=10.0, help='SQEH: surrogate spike sigmoid slope.')
    parser.add_argument('--sqeh_refractory', type=int, default=2, help='SQEH: refractory timesteps after an event fires.')
    parser.add_argument('--sqeh_density_window', type=float, default=0.08, help='SQEH: stabilizer for local density estimation in normalized time.')
    parser.add_argument('--sqeh_no_spike', type=int, default=0, help='SQEH ablation: replace event spikes with always-on event gates.')
    parser.add_argument('--sqeh_no_quat_relation', type=int, default=0, help='SQEH ablation: replace Hamilton relation routing with dot-product routing.')
    parser.add_argument('--sqeh_static_field', type=int, default=0, help='SQEH ablation: disable density-adaptive event field.')
    parser.add_argument('--sqeh_softmax_route', type=int, default=0, help='SQEH ablation: use per-node softmax routing.')
    parser.add_argument('--sqeh_no_node_attn', type=int, default=0, help='SQEH ablation: disable node self-attention stabilizer.')
    parser.add_argument('--sqeh_no_decoder_ctx', type=int, default=0, help='SQEH ablation: disable scatter time/variable decoder context.')
    parser.add_argument('--sqeh_lambda_div', type=float, default=0.05, help='SQEH: event diversity loss weight.')
    parser.add_argument('--sqeh_diag_interval', type=int, default=0, help='SQEH: log event routing diagnostics every N forward passes (0=disabled).')
    parser.add_argument('--sqeh_window_size', type=int, default=5, help='SQEH: window size for TemporalEventConvolution.')
    parser.add_argument('--sqeh_var_interaction', type=int, default=1, help='SQEH: enable VariableInteraction (0=disabled, 1=enabled).')
    # ReIMTS
